...
/**
 * nghiaht (nghiaht.github.io)
 * Ported from http://www.informatik.uni-leipzig.de/~duc/amlich/VietCalendar.java
 */
namespace LichDeBan.Helpers
{
     ...
     */
         ...
         */
        public static LunarInfo GetLunarInfo(DateTime date)
        {
            int[] lunarArr = convertSolar2Lunar(date.Day, date.Month, date.Year, 7.0);
            return new LunarInfo
            {
                Day = lunarArr[0],
                Month = lunarArr[1],
                Year = lunarArr[2],
                IsLeapMonth = lunarArr[3] == 1,
                CanChi = CanChiHelper.GetCanChiYear(lunarArr[2])
            };
        }
